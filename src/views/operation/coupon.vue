      ...
      <div class="rightbox">
        <el-row>
          <el-col :span="24">
            <el-tabs v-model="tabStatus" @tab-click="changeTab">
              <el-tab-pane
                :label="$t('operation.puTongCoupon')"
                name="coupon"
              />

              <el-tab-pane :label="$t('operation.liveCoupon')" name="live" />
            </el-tabs>
          </el-col>
        </el-row>
        ...
        >
          <el-col :span="14">
            ...
            <span
              v-if="tabStatus === 'live'"
              style="font-size: 14px;color: #020;margin-left: 10px;margin-right: 5px"
            >
              {{ $t("operation.viewIssued") }}
            </span>
            ...
            <div style="height: calc(100vh - 235px)">
              ...
              >
                ...
                >
                  <template slot-scope="scope">
                    <el-button
                      @click="showCouponEditor(scope.row)"
                      size="small"
                    >
                      {{ $t("tools.edit") }}
                    </el-button>
                    ...
                    <delete-button
                      ...
                      @delData="deleteCouponFunc(scope.row)"
                    />
                  </template>
                </el-table-column>
              </el-table>
          ...
          <el-form :model="form" label-position="top">
            ...
            <el-row type="flex" justify="space-between" align="center">
              ...
              <el-col :span="7">
                <el-form-item
                  v-if="form.type === 1"
                  :label="$t('operation.couponMoney')"
                >
                  <price-input v-model="form.quantity_discount_value" />
                </el-form-item>

                <el-form-item
                  v-else-if="form.type === 2"
                  :label="$t('operation.couponDiscount')"
                >
                ...
                <el-form-item
                  ...
                  :label="$t('operation.exchangeGoods')"
                >
                  <goods-selector
                    :mulit="true"
                    v-model="form.exchange_goods_ids"
                  />
                </el-form-item>
              </el-col>
              ...
              <el-col :span="8">
                <el-form-item
                  v-if="form.range_type === 2"
                  :label="$t('operation.rangeGoods')"
                >
                  <goods-selector :mulit="true" v-model="form.goods_spu_ids" />
                </el-form-item>

                <el-form-item
                  v-else-if="form.range_type === 3"
                  :label="$t('operation.rangeTypes')"
                >
                  <el-cascader
            ...
            <el-form-item :label="$t('operation.perCount')">
              <el-input v-model.number="form.per_quota_count">
                <template slot="append">
                  {{ $t("operation.perUser") }}
                </template>
              </el-input>
            ...
            <el-form-item v-if="validType" :label="$t('operation.validTime2')">
              <el-input v-model.number="form.valid_day_count">
                <template slot="prepend">
                  {{ $t("operation.validTime2Tip1") }}
                </template>
  ...
  watch: {
    ...
    selectedType(val) {
      if (val === this.$t("tools.all")) {
        ...
        const result = this.couponType.find(item => {
  ...
  methods: {
    ...
    setForm(data) {
      if (data) {
        ...
        temp.goods_spu_ids = temp.goods_spu_ids
          ? JSON.stringify(temp.goods_spu_ids)
          : "";
        temp.exchange_goods_ids = temp.exchange_goods_ids
          ? Array.isArray(temp.exchange_goods_ids)
            ? JSON.stringify(temp.exchange_goods_ids)
            : temp.exchange_goods_ids
          : "";
    ...
    saveAdFunc() {
      ...
      if (this.form.id !== "") {
        couponModify(this.form.id, this.form)
          .then(res => {
            this.getCouponListFun();
