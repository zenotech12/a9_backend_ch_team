  ...
  <div class="sys-body">
    <div class="sys-neiBody">
      ...
      <div class="rightbox">
        <el-row>
          <el-col :span="24">
            <el-tabs v-model="tabStatus">
              <el-tab-pane
                :label="$t('operation.liveComingSoon')"
                name="live"
              />

              <el-tab-pane
                :label="$t('operation.historyRecord')"
                name="history"
              />
            </el-tabs>
          </el-col>
            ...
            <el-row>
                ...
                <el-form ref="form" :model="form" label-width="150px">
                  <el-row type="flex" justify="space-between" align="top">
                    <el-col :span="12">
                      ...
                      <el-form-item
                        :label="$t('operation.liveStatus')"
                        v-if="Object.keys(currentLive).length > 0"
                      >
                        <span v-if="currentLive.status === 1">
                          {{ $t("operation.comingSoon") }}
                        </span>

                        <span v-if="currentLive.status === 2">
                          {{ $t("operation.live") }}
                        </span>
                      ...
                      <el-form-item
                        :label="$t('operation.pushStreamAddress')"
                        v-if="pushUrlShow"
                      >
                        <span>{{ pushurl }}</span>
                      </el-form-item>
              ...
              <el-col :span="24">
                <el-tabs v-model="tabGoods" @tab-click="tabChooseFunc">
                  <el-tab-pane :label="$t('operation.goods')" name="goods" />

                  <el-tab-pane
                    :label="$t('operation.liveCoupon')"
                    v-if="currentLive.status === 2"
                    name="coupon"
                  />
                </el-tabs>
              ...
              <el-col :span="24" v-if="tabGoods === 'goods'">
                <el-row>
                  ...
                  <el-col
                    :span="24"
                    style="text-align: right;padding: 10px 15px"
                    v-if="permissionCheck('opt') && currentLive.status === 2"
                  >
                    <div class="boxFuncBtn">
                      <goods-selector
                        ...
                        v-if="modifyDisabled"
                        :trigger="false"
                        :haveTime.sync="modifyDisabled"
                        :showTag="!modifyDisabled"
                        v-model="spu_ids"
                        :notSpuIds="JSON.stringify(currentLive.spu_ids)"
                        @selectChanged="addNewGoods"
                        :approve_status="2"
                        :shelf_status="2"
                      />
                  ...
                  >
                    <div class="boxFuncBtn">
                      <goods-selector
                        ...
                        :showTag="modifyDisabled"
                        :trigger="false"
                        v-model="form.spu_ids"
                        @selectChanged="selectChanged"
                        :approve_status="2"
                        :notSpuIds="JSON.stringify([])"
                        :shelf_status="2"
                      />
                    </div>
                  </el-col>
                          ...
                          <template slot-scope="scope">
                            ...
                            >
                              <el-image
                                ...
                                :src="
                                  getImageUrl(scope.row.images[0], 100, 100)
                                "
                                fit="cover"
                              />
                              <div class="g-info">
                        ...
                        >
                          <template slot-scope="scope">
                            <el-switch
                              ...
                              @change="
                                explainFunc(scope.row.id, scope.row.explain)
                              "
                              active-color="#13ce66"
                              inactive-color="#ff4949"
                            />
                        ...
                        <el-table-column :label="$t('tools.opt')" width="200">
                          <template slot-scope="scope">
                            <i
                              ...
                              v-if="!modifyDisabled"
                              @click="delCurrentGoods(scope.row.id)"
                            />

                            <el-button
                              size="small"
                              v-if="
                                currentLive.status === 2 &&
                                  scope.row.recommend === false &&
                                  scope.row.explain === false
                              "
                              @click="goodsOffShelf(scope.row.id, scope.$index)"
                            >
                              {{ $t("operation.goodsOffshelf") }}
                            </el-button>
                          </template>
                        </el-table-column>
                      </el-table>
                ...
                <el-row style="margin-top: 20px;">
                  <el-col :span="24">
                    <confirm-button
                      ...
                      v-if="type === 'add' && permissionCheck('opt')"
                      :confirmButtonInfor="$t('tools.save')"
                      btnType="success"
                    />

                    <confirm-button
                      @confirmButton="editLive"
                      :disabled="submitDisabled"
                      btnType="success"
                      v-if="
                        type === 'edit' &&
                    ...
                    <delete-button
                      :promptInfor="delTip"
                      v-if="currentLive.status === 1 && permissionCheck('opt')"
                      btnType="danger"
                      @delData="delCurrentLive"
                    />
                    ...
                    <el-button
                      ...
                      size="small"
                      @click="liveStartFunc"
                      v-if="isShowStart && permissionCheck('opt')"
                    >
                      {{ $t("operation.startBroadcast") }}
                    </el-button>
                    ...
                    <el-button
                      ...
                      size="small"
                      @click="liveStopFunc"
                      v-if="currentLive.status === 2 && permissionCheck('opt')"
                    >
                      {{ $t("operation.endLiveBroadcast") }}
                    </el-button>
                  </el-col>
                </el-row>
              </el-col>
                    ...
                    <div style="height: calc(100vh - 490px)">
                      ...
                      >
                        ...
                        <el-table-column :label="$t('operation.couponType')">
                          <template slot-scope="scope">
                            ...
                            <template v-if="scope.row.type === 1">
                              [{{ scope.row.quantity_discount_value | price }}]
                            </template>

                            <template v-else-if="scope.row.type === 2">
                              [{{ scope.row.percentage_discount_value }}%]
                            </template>
                          </template>
                        </el-table-column>
                        ...
                        <el-table-column :label="$t('operation.couponCond')">
                          <template slot-scope="scope">
                            {{ $t("operation.couponCondA") }}
                            {{ scope.row.threshold | price }}
                            {{ $t("operation.couponCondB") }}
                          </template>
                        </el-table-column>
                        ...
                        <el-table-column :label="$t('operation.validTime')">
                          <template slot-scope="scope">
                            <template v-if="scope.row.valid_day_count > 0">
                              {{ $t("operation.validTime2Tip1") }}

                              <strong>{{ scope.row.valid_day_count }}</strong>

                              {{ $t("operation.validTime2Tip2") }}
                            </template>
          ...
          <el-col :span="24" class="historyBox">
            <div style="height: calc(100vh - 200px)">
              ...
              >
                ...
                <el-table-column :label="$t('operation.name')" prop="name" />
                ...
                <el-table-column :label="$t('operation.cover')">
                  <template slot-scope="scope">
                    ...
                    >
                      <img
                        :src="getImageUrl(scope.row.cover_imgs[0], 100, 100)"
                        style="max-height: 480px; max-width: 600px"
                      />

                      <img
                        slot="reference"
                        :src="getImageUrl(scope.row.cover_imgs[0], 240)"
                        style="margin-right:8px;max-height: 60px;max-width: 120px"
                      />
                    </el-popover>
                  </template>
                </el-table-column>
                ...
                <el-table-column
                  :label="$t('operation.lookNumberPeople')"
                  prop="visiter_count"
                />

                <el-table-column
                  :label="$t('operation.likes')"
                  prop="thumb_count"
                />

                <el-table-column
                  :label="$t('operation.numberGoodsSold')"
                ...
                >
                  <template slot-scope="scope">
                    <el-button
                      type="text"
                      size="small"
                      @click="showOrder(scope.row, scope.row.goods_sale_count)"
                    >
                      {{ scope.row.goods_sale_count }}
                    </el-button>
                  </template>
                </el-table-column>

                <el-table-column
                  :label="$t('operation.orderNumber')"
                  prop="order_count"
                >
                  <template slot-scope="scope">
                    <el-button
                      type="text"
                      size="small"
                      @click="showOrder(scope.row, scope.row.order_count)"
                    >
        ...
        <!--订单列表-->
        <el-dialog
          :title="$t('operation.orderList')"
          width="80%"
          @close="orderShowDialog = false"
          :visible.sync="orderShowDialog"
        ...
        >
            ...
            <el-col :span="24" style="height: calc(100vh - 320px)">
              ...
              >
                ...
                <el-table-column :label="$t('order.price')" width="130">
                  <template slot-scope="scope">
                    <span
                      :title="$t('order.price1') + '+' + $t('order.price2')"
                    >
                      <template v-if="scope.row.pay_points > 0">
                        *{{ scope.row.pay_points }}+
                      </template>

                      {{ scope.row.pay_price | price }}
                    </span>
                  ...
                  <template slot-scope="scope">
                    ...
                    <div class="ui" v-if="scope.row.express.novar">
                      ...
                      <el-popover
                        ...
                        trigger="click"
                        v-if="
                          scope.row.express.company === 'zto' ||
                            scope.row.express.company === 'rider'
                        "
                      >
                        <div v-if="expressInfo && !showOrderStatus">
                          <el-timeline
                            style="margin-top: 10px"
                            v-if="expressInfo.length > 0"
                          >
                            <el-timeline-item
                          ...
                          <p
                            v-else
                            style="color: #333; font-size: 14px; text-align: center; font-weight: bold"
                          >
                            {{ $t("order.zwddwl") }}
                          </p>
                        </div>
                        ...
                        <div v-if="showOrderStatus">
                          <el-timeline style="margin-top: 10px">
                            <el-timeline-item
                              v-for="(record, index) in scope.row
                                .operation_records"
                              :key="index"
                              :timestamp="record.time"
                            >
          ...
          <div slot="footer" class="dialog-footer">
            <el-button
              @click="orderShowDialog = false"
              size="small"
              style="margin-right: 24px;margin-left: 10px;"
            >
              {{ $t("tools.close") }}
            </el-button>
          </div>
        ...
        <!--优惠券-->
        <el-dialog
          :title="$t('operation.chooseCoupon')"
          width="40%"
          @close="couponShowDialog = false"
        ...
        >
          ...
          <div slot="footer" class="dialog-footer">
            <el-button @click="saveCoupon" size="small" type="primary">
              {{ $t("tools.confirm") }}
            </el-button>

            <el-button
              @click="couponShowDialog = false"
              size="small"
              style="margin-right: 24px;margin-left: 10px;"
            >
              {{ $t("tools.close") }}
...
export default {
  ...
  data() {
    ...
    return {
      ...
      liveCouponList: [],
      couponType: [
        { code: 1, name: this.$t("operation.couponTypeA") },
        { code: 2, name: this.$t("operation.couponTypeB") },
        { code: 3, name: this.$t("operation.couponTypeC") }
      ],
      rangeType: [
        { code: 1, name: this.$t("operation.rangeTypeA") },
        { code: 2, name: this.$t("operation.rangeTypeB") },
        { code: 3, name: this.$t("operation.rangeTypeC") }
      ]
    };
  },
  computed: {
  ...
  methods: {
    ...
    liveStartFunc() {
      ...
      liveItemsList(this.searchForm).then(res => {
        const obj = res.items[0]
          ? JSON.parse(JSON.stringify(res.items[0]))
          : {};
        if (obj) {
          const item = {
    ...
    lookLiveVideo() {
      this.addResources();
      this.videoUrl = this.currentLive.channel_info.hlsPullUrl.replace(
        "http:",
        ""
      );
    ...
    addNewGoods(data) {
      ...
      liveItemsAddGoods(this.currentLive.id, { spu_ids: this.spu_ids }).then(
        res => {
          this.spu_ids = "";
          this.getLiveListFunc();
        }
    ...
    showOrder(data, number) {
      if (number > 0) {
        ...
        this.orderSearch.spu_ids = data.spu_ids
          ? JSON.stringify(data.spu_ids)
          : "";
        this.getOrderList();
    ...
    saveAdFunc() {
      ...
      if (this.type === "edit") {
        liveItemsModify(this.currentLive.id, this.form)
          .then(res => {
            this.getLiveListFunc();
    ...
    getLiveGoodsList() {
      spusList({ spu_ids: JSON.stringify(this.currentLive.spu_ids) }).then(
        res => {
...
.liveVideo {
  /deep/ .my-video-dimensions.vjs-fluid {
    padding-top: 0;
  }
}
.liveBox {
