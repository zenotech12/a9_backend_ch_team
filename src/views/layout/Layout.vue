...
export default {
  ...
  computed: {
    ...
    ...mapGetters(["imInfo", "shopInfo", "token"])
  },
  beforeDestroy() {
    this.socket.close();
  },
  methods: {
    ...
    socketOpen() {
      // 链接成功 注册信息
      this.socket.send(
        JSON.stringify({ type: 1, body: { token: this.token } })
      );
      // this.socketPing()
    },
    socketError() {
      ...
      if (
        ...
        if (window.Notification.permission === "granted") {
          ...
          _this.notification.onclick = function() {
            // alert('通知被点击')
            if (
              this.permissionCheck("view", "3_1") ||
              this.permissionCheck("view", "3_2") ||
              this.permissionCheck("view", "3_4")
            ) {
              let pushUrl = {};
  ...
  created() {
    ...
    if (lang !== "zh" || currentLang !== "zh") {
      ...
      oScript.src =
        "https://maps.googleapis.com/maps/api/js?key=AIzaSyCNmBY7o9y-mK6omSYieGg12TE25xjqsms&libraries=places";
