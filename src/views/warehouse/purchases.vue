  ...
  <div class="sys-body">
    <div class="sys-neiBody">
      ...
      <div class="rightbox">
            ...
            <el-form :inline="true" :model="searchForm">
              ...
              <el-form-item>
                <el-select
                  ...
                  :placeholder="$t('warehouse.pleaseChoose')"
                >
                  <el-option
              ...
              <el-form-item>
                <el-input
                  ...
                  :placeholder="$t('warehouse.pleaseEnterNo')"
                  clearable
                  style="width: 260px"
                ></el-input>
              </el-form-item>
          ...
          <el-col :span="24">
            <div style="height: calc(100vh - 226px)">
              <el-table
                border
                :data="tableData"
                @selection-change="exportChangeChoose"
              ...
              >
                ...
                <el-table-column
                  prop="no"
                  :label="$t('warehouse.Singlenumber')"
                  width="150"
                  fixed="left"
                  align="center"
                ></el-table-column>
                <el-table-column
                  :label="$t('warehouse.type2')"
                  width="180px"
                  align="center"
                >
                  <template slot-scope="scope">
                    <el-tag
                      ...
                      >{{ $t("warehouse.pendFinancialApproval") }}</el-tag
                    >
                    <el-tag type="info" v-if="scope.row.status === 2">{{
                      $t("warehouse.leaderBeApproved")
                    }}</el-tag>
                    <el-tag type="warning" v-if="scope.row.status === 3">{{
                      $t("warehouse.tobeStored")
                    }}</el-tag>
                    <el-tag v-if="scope.row.status === 4">{{
                      $t("warehouse.compleateStorage")
                    }}</el-tag>
                    <el-tag type="danger" v-if="scope.row.status === 100">{{
                      $t("tools.cancel")
                    }}</el-tag>
                  </template>
                </el-table-column>
                <el-table-column width="1250">
                  <template slot-scope="scope" slot="header">
                    <el-row style="width: 100%">
                      <el-col :span="5">{{ $t("warehouse.name2") }}</el-col>
                ...
                <el-table-column
                  :label="$t('warehouse.Approvalrecord')"
                  width="150"
                  align="center"
                >
                  <template slot-scope="scope">
                          ...
                          <div class="ui">
                            <div>
                              ...
                              <span v-if="record.status === 2">{{
                                $t("global.financialApproval")
                              }}</span>
                              <span v-if="record.status === 3">{{
                                $t("global.leadershipApproval")
                              }}</span>
                              <span v-if="record.status === 4">{{
                                $t("warehouse.Completereceipt")
                              }}</span>
                              <span v-if="record.status === 100">{{
                                $t("tools.cancel")
                              }}</span>
                            </div>
                            <div>
                ...
                <el-table-column
                  prop="payment_term"
                  v-if="permissionCheck('opt', '9_3')"
                  :label="$t('warehouse.payment_term')"
                  width="135"
                  align="center"
                ></el-table-column>
                <el-table-column
                  prop="delivery_method"
                  :label="$t('warehouse.delivery_method')"
                  width="140"
                  align="center"
                ></el-table-column>
                <el-table-column
                  :label="$t('warehouse.payment')"
                  v-if="permissionCheck('opt', '9_3')"
                  width="240"
                  align="center"
                >
                ...
                <el-table-column
                  :label="$t('warehouse.PaymentAmount')"
                  v-if="permissionCheck('opt', '9_3')"
                  width="150"
                  align="center"
                >
                ...
                <el-table-column
                  :label="$t('warehouse.Pallprice')"
                  v-if="permissionCheck('opt', '9_3')"
                  width="170"
                  align="center"
                >
                  <template slot-scope="scope">
                    <span class="bold-text">{{ scope.row.money | price }}</span>
                  </template>
                </el-table-column>
                <el-table-column
                  prop="estimate_remain_days"
                  :label="$t('warehouse.estimate_remain_days')"
                  width="170"
                  align="center"
                ></el-table-column>
                <el-table-column
                  prop="estimate_arrival_time"
                  :label="$t('warehouse.estimate_arrival_time')"
                  width="160"
                  align="center"
                ></el-table-column>
                <el-table-column
                  prop="gen_time"
                  :label="$t('warehouse.OrderTime')"
                  width="160"
                  align="center"
                ></el-table-column>
                <el-table-column
                  ...
                  v-if="
                    ...
                    permissionCheck('opt', '9_1') ||
                    permissionCheck('opt', '9_2')
                  "
                  fixed="right"
                  align="center"
                >
                  <template slot-scope="scope">
                    <div style="display: flex; flex-direction: column">
                      <el-button
                        type="text"
                        @click="showDataEditor(scope.row, true)"
                        v-if="
                          permissionCheck('opt', '8_3') &&
                          scope.row.status === 100
                        "
                        size="small"
                        >{{ $t("warehouse.repurchase") }}</el-button
                      >
                      ...
                      <el-button
                        type="text"
                        @click="showDataEditor(scope.row, false)"
                        v-if="
                          (permissionCheck('opt', '8_3') &&
                            scope.row.status <= 1) ||
                          (permissionCheck('opt', '9_1') &&
                      ...
                      <el-button
                        type="text"
                        @click="shengheFunc(scope.row.id, 2)"
                        v-if="
                          permissionCheck('opt', '9_1') &&
                          (scope.row.status === 1 || scope.row.status === 0)
                        "
                        size="small"
                        >{{ $t("global.financialApproval") }}</el-button
                      >
                      <el-button
                        type="text"
                        @click="shengheFunc(scope.row.id, 3)"
                        v-if="
                          permissionCheck('opt', '9_2') &&
                          scope.row.status === 2
                        "
                        size="small"
                        >{{ $t("global.leadershipApproval") }}</el-button
                      >
                      <el-button
                        type="text"
                        @click="shengheFunc(scope.row.id, 100)"
                        v-if="
                          (permissionCheck('opt', '8_3') &&
                            scope.row.status <= 1) ||
                          (permissionCheck('opt', '9_1') &&
                        ...
                        "
                        size="small"
                        >{{ $t("tools.cancel") }}</el-button
                      >
                      <!--<span class="xiexian">/</span>-->
                      <el-button
                        type="text"
                        @click="paidListFunc(scope.row)"
                        v-if="
                          scope.row.status === 3 &&
                          permissionCheck('opt', '9_1')
                        "
                        size="small"
                        >{{ $t("warehouse.payment2") }}</el-button
                      >
                      <!--<span class="xiexian">/</span>-->
                      <el-button
                        type="text"
                        @click="warehousing(scope.row)"
                        v-if="scope.row.status >= 3 && scope.row.status !== 100"
                        size="small"
                        >{{ $t("warehouse.Warehousing") }}</el-button
                      >
                    </div>
                  </template>
                </el-table-column>
        ...
        <!-- Dialog Purchase order settings -->
        <el-dialog
          width="90%"
        ...
        >
          ...
          <el-form :model="form" label-position="top">
            <el-row type="flex" justify="space-between" align="center">
              ...
              <el-col :span="4">
                <el-form-item :label="$t('warehouse.Currency')">
                  <base-data-select
                    ...
                    treeCode="008"
                    :placeString="$t('warehouse.Currency')"
                    style="width: 100%"
                  ></base-data-select>
                </el-form-item>
              </el-col>
              ...
              <el-col :span="4">
                <el-form-item :label="$t('warehouse.payment_term')">
                  <base-data-select
                    ...
                    treeCode="009"
                    :placeString="$t('warehouse.payment_term')"
                    style="width: 100%"
                  ></base-data-select>
                </el-form-item>
              </el-col>
              ...
              <el-col :span="4">
                <el-form-item :label="$t('warehouse.delivery_method')">
                  <base-data-select
                    ...
                    treeCode="010"
                    :placeString="$t('warehouse.delivery_method')"
                    style="width: 100%"
                  ></base-data-select>
                </el-form-item>
              </el-col>
              ...
              <el-col :span="4">
                <el-form-item :label="$t('warehouse.estimate_arrival_time')">
                  <el-date-picker
                    v-model="form.estimate_arrival_time"
                    treeCode="011"
                    format="yyyy-MM-dd"
                    value-format="yyyy-MM-dd"
                    :placeholder="$t('warehouse.estimate_arrival_time')"
                    style="width: 100%"
                  ></el-date-picker>
                </el-form-item>
              </el-col>
            ...
            <el-form-item style="margin-top: 20px">
              ...
              >
                ...
                >
                  <template slot-scope="scope">
                    <price-input
                      ...
                      @inputBlurFunc="currentIputCount(scope.$index)"
                      ><span slot="append"
                ...
                >
                  <template slot-scope="scope">
                    <el-input
                      ...
                      @blur="currentIputCount(scope.$index)"
                    ></el-input>
                  </template>
                </el-table-column>
                <el-table-column
                ...
                <el-table-column
                  prop="other_price"
                  :label="$t('warehouse.otherPrice')"
                  width="150"
                  align="center"
                >
                  <template slot-scope="scope">
                    <price-input-two
                      v-model="scope.row.other_price"
                    ></price-input-two>
                  </template>
                </el-table-column>
                <el-table-column
        ...
        <!-- Dialog Purchase settings -->
        <el-dialog
          width="80%"
          append-to-body
          @close="skusDialog = false"
          :visible.sync="skusDialog"
          :close-on-click-modal="false"
          center
        >
          <span slot="title" style="font-weight: bold; font-size: 15px">{{
            $t("warehouse.PurchaseSet")
          }}</span>
          <el-form label-width="70px" label-position="left">
            ...
            <el-form-item v-show="source === 2" label-width="0">
              <!-- :label="$t('warehouse.orderinformation')" -->
              <el-table
                stripe
                border
                :data="orderInfoData"
                ref="orderInfoDataTable"
                key="1"
              >
                <el-table-column :label="$t('order.no')" width="220px">
                  <template slot-scope="scope">
                    <el-tag
                      style="display: block; width: 50px; margin: 0 auto"
                      v-if="scope.row.type === 2"
                      size="mini"
                      >{{ $t("order.orderType2") }}</el-tag
                    >
                    <el-tag
                      style="display: block; width: 60px; margin: 0 auto"
                      v-if="scope.row.type === 3"
                      size="mini"
                      >{{ $t("order.orderType3") }}</el-tag
                    >
                    <el-tag
                      style="display: block; width: 60px; margin: 0 auto"
                      v-if="scope.row.type === 4"
                      size="mini"
                      >{{ $t("order.orderType4") }}</el-tag
                    >
                    No:{{ scope.row.no }}<br />
                    <span class="f12" v-if="scope.row.pay_id"
                      >Id:{{ scope.row.pay_id }}</span
                    ><br />
                    <el-popover
                      v-if="
                        scope.row.comment ||
                ...
                <!-- 金额 -->
                <el-table-column :label="$t('order.price')" width="250">
                  <template slot-scope="scope">
            ...
            <el-form-item v-if="source === 1" label-width="0">
              <el-table
                ref="multipleTable"
                :data="goodsInventoryTable"
                row-key="id"
                key="1"
          ...
          <div slot="footer" class="dialog-footer">
            <confirm-button
              @confirmButton="saveDataFuncSkus()"
              :disabled="submitDisabled"
              :confirmButtonInfor="$t('warehouse.save')"
            ></confirm-button>
            <el-button @click="skusDialog = false" size="small">{{
              $t("tools.cancel")
            }}</el-button>
          </div>
        </el-dialog>
        ...
        <!-- 付款单列表 -->
        <!-- Dialog Payment Slip list  -->
        <el-dialog
          :visible.sync="paidListDialog"
          width="80%"
          @close="paidListDialog = false"
          :close-on-click-modal="false"
          center
        >
          <span slot="title" style="font-weight: bold; font-size: 15px">{{
            $t("warehouse.paymentslist")
          }}</span>
          ...
          <div slot="footer" class="dialog-footer">
            ...
            <confirm-button
              @confirmButton="Paycomplete(2)"
              :disabled="submitDisabled"
              :confirmButtonInfor="$t('warehouse.close')"
              btnType="primary"
            ></confirm-button>
          </div>
        </el-dialog>
        <!-- 付款单设置 -->
        <!-- Dialog Payment Slip Setting -->
        <el-dialog
          width="700px"
          append-to-body
          @close="payaddDialog = false"
          :visible.sync="payaddDialog"
          :close-on-click-modal="false"
          center
        >
          <span slot="title" style="font-weight: bold; font-size: 15px">{{
          ...
          <el-form label-width="150px" :model="skus" label-position="left">
            ...
            <el-form-item :label="$t('goods.note')" label-width="90px">
              <el-input type="textarea" :rows="3" v-model="paiForm.comment">
              </el-input>
            </el-form-item>
            <el-form-item :label="$t('warehouse.annex')" label-width="90px">
              <el-upload
                name="image"
          ...
          <div slot="footer" class="dialog-footer">
            <confirm-button
              ...
              :confirmButtonInfor="$t('tools.confirm')"
            ></confirm-button>
            <el-button @click="payaddDialog = false" size="small">{{
              $t("tools.cancel")
            }}</el-button>
          </div>
        </el-dialog>
        ...
        <el-dialog
          width="95%"
          @close="inwarehouselog = false"
          :visible.sync="inwarehouselog"
          :close-on-click-modal="false"
          center
        >
          <span slot="title" style="font-weight: bold; font-size: 15px">{{
            $t("warehouse.Warehousing")
          }}</span>
          <el-select
            v-model="wareId"
            @change="onchange"
          ...
          <el-switch
            ...
            :inactive-text="$t('order.notArrived')"
            active-color="#13ce66"
            inactive-color="#ff4949"
          ...
          <el-switch
            ...
            v-if="wareId"
            v-model="switchtype"
            :inactive-text="$t('warehouse.Tobetested')"
            active-color="#13ce66"
            inactive-color="#ff4949"
          ></el-switch>
          ...
          <div v-if="wareId">
            ...
            >
              ...
              <el-table-column :label="$t('warehouse.barCode')" width="174px">
                <template slot-scope="scope">
                  <el-input v-model="scope.row.barcode"></el-input>
                </template>
              </el-table-column>
              <el-table-column :label="$t('warehouse.location')" width="110px">
                <template slot-scope="scope">
                  <el-select
              ...
              <el-table-column :label="$t('warehouse.baozhiqi')" width="130px">
                <template slot-scope="scope">
                  <el-input v-model.number="scope.row.shelf_life" clearable>
                  </el-input>
                </template>
              </el-table-column>
              <el-table-column :label="$t('warehouse.guoqiTime')" width="110px">
                <template slot-scope="scope">
                  <el-date-picker
              ...
              <!-- 本次到货 已到货 采购数量 -->
              <el-table-column
                :label="$t('warehouse.thearrivalarr')"
                width="296px"
                align="center"
              >
                <template slot-scope="scope">
              ...
              <div
                slot="append"
                v-if="inwarehouseData.length === 0 && !filterData"
                class="tipMsg"
              >
                {{ $t("warehouse.noGoodsTipLook") }}
                <el-button type="text" @click="filterData = true">{{
                  $t("tools.check")
                }}</el-button>
              </div>
            </el-table>
            <div style="margin-top: 10px">
              <el-input
                :placeholder="$t('warehouse.remarks')"
                type="textarea"
                :rows="3"
                v-model="inwarehouseFrom.comment"
              ></el-input>
            </div>
          </div>
          ...
          <div slot="footer" class="dialog-footer">
            <confirm-button
              @confirmButton="inwarehouseAdd"
              :disabled="submitDisabled"
              :confirmButtonInfor="$t('tools.confirm')"
            ></confirm-button>
            <el-button @click="inwarehouselog = false" size="small">{{
              $t("tools.cancel")
            }}</el-button>
          </div>
        </el-dialog>
        ...
        <el-dialog :visible.sync="dialogVisible" width="90%">
          ...
          <el-tabs v-model="activeChuRuKu" @tab-click="tabChange">
            ...
            ></el-tab-pane>
            <el-tab-pane
              :label="$t('warehouse.Returntolist')"
              name="2"
            ></el-tab-pane>
          </el-tabs>
          <div v-if="activeChuRuKu === '1'">
            <el-row>
              ...
              <el-col
                :span="24"
                class="funcList localcss"
                style="display: flex"
                v-if="permissionCheck('opt', '8_1')"
              >
                <el-button
                  type="success"
                  class="overbtn"
                  ...
                  >{{ $t("warehouse.Completereceipt") }}</el-button
                >
                ...
                <el-button
                  ...
                  size="small"
                  class="overbtn"
                  style="right: 0px; padding: 10px 20px"
                  icon="el-icon-plus"
                  @click="addrukudata"
                  >{{ $t("tools.add") }}</el-button
                >
            ...
            >
              ...
              <el-table-column prop="warehouse_name" width="1450px">
                <template slot="header" slot-scope="scope">
                  <div>
                    <el-row style="width: 100%">
                      ...
                      <el-col :span="2" style="text-align: center">{{
                        $t("warehouse.returnednum")
                      }}</el-col>
                      <el-col :span="4" style="text-align: center">{{
                        $t("warehouse.baozhiqiguoqi")
                      }}</el-col>
                    </el-row>
                  </div>
                </template>
                <template slot-scope="scope">
                  <div class="goods">
                    <el-row
                      ...
                      style="width: 100%"
                    >
                      <el-col :span="5" class="overOmitted"
                        ><span :title="item.name">{{ item.name }}</span></el-col
                      >
                      <el-col
                        :span="3"
                      ...
                      <el-col :span="4" style="text-align: center"
                        >{{ item.shelf_life }}{{ $t("warehouse.day") }} /
                        {{ item.exp }}</el-col
                      >
                    </el-row>
                  </div>
          ...
          <div v-if="activeChuRuKu === '2'">
            ...
            >
              ...
              <el-table-column width="1000">
                <template slot="header" slot-scope="scope">
                  <el-row style="width: 100%">
                    <el-col :span="7">{{ $t("warehouse.Tradename") }}</el-col>
                    <el-col :span="2" style="text-align: center">{{
                ...
                <template slot-scope="scope">
                  <div class="goods">
                    <el-row
                      ...
                      style="width: 100%"
                    >
                      <el-col :span="7" class="overOmitted"
                        ><span :title="item.name">{{ item.name }}</span></el-col
                      >
                      <el-col
                        :span="2"
              ...
              <el-table-column
                ...
                fixed="right"
                v-if="
                  permissionCheck('opt', '8_1') || permissionCheck('opt', '8_3')
                "
                align="center"
              >
                <template slot-scope="scope">
        ...
        <!-- Warehousing Dialog -->
        <el-dialog :visible.sync="Rudalog" width="75%">
          <span slot="title" style="font-weight: bold; font-size: 15px">{{
            $t("warehouse.Warehousing")
          }}</span>
          ...
          ><br />
          <p style="font-weight: bold">{{ $t("warehouse.remarks") }}</p>
          <el-input
            type="textarea"
            :rows="3"
            v-model="Rukufrom.comment"
            :placeholder="$t('warehouse.remarks')"
          ></el-input>
          <span slot="footer" class="dialog-footer">
            <el-button type="success" size="small" @click="Rukuadd">{{
              $t("warehouse.confirmmsg")
            }}</el-button>
            <el-button @click="Rudalog = false" size="small">{{$t('tools.cancel')}}</el-button>
          </span>
        </el-dialog>
        <!-- 退还 -->
        <el-dialog :visible.sync="returndalog" width="75%">
          <span slot="title" style="font-weight: bold; font-size: 15px">{{
            $t("warehouse.Info")
          }}</span>
          <el-table
          ...
          <span slot="footer" class="dialog-footer">
            <el-button type="success" size="small" @click="returnbtn">{{
              $t("warehouse.confirmmsg")
            }}</el-button>
            <el-button @click="returndalog = false" size="small">{{
              $t("tools.cancel")
            }}</el-button>
          </span>
        </el-dialog>
        <!--审批备注 -->
        <el-dialog :visible.sync="shengpiNoteDialog" width="40%">
          <span slot="title" style="font-weight: bold; font-size: 15px">{{
            $t("warehouse.Approve")
          }}</span>
          <el-form
          ...
          <span slot="footer" class="dialog-footer">
            <el-button type="success" size="small" @click="shengheSubmit">{{
              $t("warehouse.confirmmsg")
            }}</el-button>
            <el-button @click="shengpiNoteDialog = false" size="small">{{
              $t("tools.cancel")
            }}</el-button>
          </span>
        </el-dialog>
...
import {
  purchaseover,
  warehousePurchasesExport,
  warehouseOutboundReview,
  warehouseOutbounds,
  receiptsinventory,
  warehouseOutboundsAdd,
  purchaseAdd,
  warehousePurchasesCount,
  warehousePurchasesReview,
  Locationlist,
...
export default {
  ...
  data() {
    ...
    return {
      ...
      statusTab: [
        { value: "0", label: this.$t("tools.all") },
        ...
        skus.forEach((item) => {
          ...
          for (let i = 0; i < this.goodsInventoryData.length; i++) {
            ...
            for (let j = 0; j < keys.length; j++) {
              if (
                this.goodsInventoryData[i].specifications[keys[j]] !==
                item[keys[j]]
              ) {
                isEque = false;
                break;
            ...
            if (isEque) {
              ...
              tableItem.total_price =
                this.goodsInventoryData[i].inventory *
                this.goodsInventoryData[i].price;
            }
          }
  ...
  methods: {
    currentIputCount(index) {
      ...
      this.$set(
        this.skusArray[index],
        "total_price",
        this.skusArray[index].count * this.skusArray[index].unit_price
      );
    ...
    exportExcel() {
      ...
      warehousePurchasesExport({ ids: JSON.stringify(this.purchasesIds) }).then(
        (res) => {
          ...
          res.items.forEach((item) => {
            this.statusTab.forEach((Z, k) => {
              if (Z.value === item.status.toString()) {
                this.statusTab[k].label =
                  this.statusTab[k].label + "(" + item.count + ")";
              }
            });
          });
        }
    ...
    shengheSubmit() {
      warehousePurchasesReview(this.shengheForm.id, this.shengheForm).then(
        (res) => {
    ...
    getGoodsInfo(id) {
      spusInfo(id).then((res) => {
        this.goodsProps = [];
        if (res.meta === 0) {
          res.item.specification_options &&
            res.item.specification_options.forEach((op) => {
              this.goodsProps.push({
                name: op.name,
                items: op.items,
                goodsName: res.item.name,
              });
            });
        }
      });
    ...
    getTreePath(k) {
      const path = [];
      if (
        k > this.goodsProps.length - 1 ||
        !this.goodsProps[k] ||
        !this.goodsProps[k].items ||
        this.goodsProps[k].items < 1
      ) {
        return path;
      }
              ...
              const obj = {
                id:
                  (goodsinfo.sku_url !== ""
                    ? goodsinfo.sku_url
                    : goodsinfo.sku_id) +
                  this.textFilter1(goodsinfo.specifications),
    ...
    saveDataFunc() {
      ...
      // console.log('this.skusArray', this.skusArray)
      if (this.form.id !== "") {
        purchaseModify(this.form.id, this.form)
          .then((res) => {
            this.getDataListFun();
    ...
    inwarehouseAdd() {
      ...
      if (this.flag == true) {
        this.flag2 = this.inwarehouseFrom.skus.some((item) => {
          return (
            item.merchant_type_code.length == 0 ||
            item.merchant_type_code == null
          );
        });
        if (this.flag3) {
          if (this.flag2 == false) {
            this.inwarehouseFrom.skus.forEach((item) => {
              const array = [];
              array[0] =
                item.merchant_type_code[item.merchant_type_code.length - 1];
              item.merchant_type_code = array;
            });
            this.inwarehouseFrom.skus = JSON.stringify(
              this.inwarehouseFrom.skus
            );
            warehouseReceiptsAdd(this.inwarehouseFrom)
              .then((res) => {
                if (res.meta == 0) {
              ...
              .catch((res) => {
                this.inwarehouseFrom.skus = JSON.parse(
                  this.inwarehouseFrom.skus
                );
              });
          } else {
    ...
    Rukuadd() {
      ...
      if (this.flag == true) {
        ...
        receiptsinventory(this.rukuid, this.Rukufrom)
          .then((res) => {
            this.Rudalog = false;
            // this.dialogVisible = false
            this.getstockinfo();
...
.add-btn {
  ...
  font-size: 14px;
  color: white;
  box-shadow: 0 0 7px rgb(110, 110, 110);
  // color: #88898a;
  // &:hover{
  //   color: rgb(30, 66, 121);
  // }
  &:hover {
    background-color: #b3d35d;
  }
  &:active {
    background-color: #a5cb38;
  }
