<template>
    <div>
        <el-input readonly placeholder="请选择订单" clearable v-model="orderInfo.name">
        ...
        <el-dialog title="订单列表" width="80%" :visible.sync="dialogFormVisible" center append-to-body>
            ...
            <el-col :span="24">
              <div>
                <el-table stripe border :data="tableData" :row-key="productRowKey" height="calc(100vh - 390px)" @current-change="selectionChange">
                  <!-- 单号 -->
...
<script>
  ...
  export default {
    ...
    watch: {
      ...
      productSelectTypes(val) {
        this.searchForm.code = val[ val.length - 1]
      },
      goodId(val) {
